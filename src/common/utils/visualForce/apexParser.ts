...
export class ApexParser {
  ...
  //private static SINGLE_LINE_COMMENT_REGEX = /\/\/\s*(.*)/; // For properties/methods without full Javadoc
  private static METHOD_REGEX = /(public|private|global|protected)\s+(?:(static|virtual|override|abstract|testMethod)\s+)*(?:<[^>]+>\s+)?([a-zA-Z0-9_<>.[\]]+)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*(?:throws\s+[^{]+)?\s*(\{)?/;
  ...
  public static async parse(apexContent: string, className: string): Promise<ApexParsedInfo> {
    ...
    const classJavadocMatch = apexContent.match(new RegExp(`^\\s*${this.JAVADOC_REGEX.source}\\s*(?:public|private|global|protected)\\s+(?:with\\s+sharing|without\\s+sharing)?\\s*class\\s+${className}`, 's'));
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const javadocMatch = line.match(/^\/\*\*(.*?)\*\/$/s);
      if (javadocMatch) {
        currentJavaDoc = [javadocMatch[1]];
        continue;
      }
      if (line.startsWith('/**')) {
        currentJavaDoc = [];
      ...
      if (methodMatch) {
        const modifiers = [methodMatch[2]].filter(Boolean);
      ...
      if (propertyMatch) {
        const getterSetter = propertyMatch[5] || '';
        result.properties.push({
          name: propertyMatch[4],
          type: propertyMatch[3],
          accessModifier: propertyMatch[1],
          hasGetter: getterSetter.includes('get'),
          hasSetter: getterSetter.includes('set'),
          javaDoc: currentJavaDoc.length > 0 ? this.cleanJavadoc(currentJavaDoc.join('\n')) : undefined,
          line: i + 1,
          isStatic: !!propertyMatch[2]?.includes('static'),
          isFinal: !!propertyMatch[2]?.includes('final')
        });
        currentJavaDoc = [];
        continue;
      }
      ...
      if (currentJavaDoc.length > 0 && line && !line.startsWith('*') && !line.startsWith('//')) {
        currentJavaDoc = [];
      }
    }
  ...
  public static formatForPrompt(
    ...
    if (classJavadoc) {
      lines.push(`// Description: ${classJavadoc.split('\n')[0]}`);
    }
  ...
  private static formatMethod(method: ApexMethod): string {
    ...
    if (method.javaDoc) {
      methodLine += ` // ${method.javaDoc.split('\n')[0].substring(0, 100)}`; // Limit length
    }
